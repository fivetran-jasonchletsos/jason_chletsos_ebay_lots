#!/usr/bin/env bash
# =============================================================================
# deploy_boiler_alarm.sh — ship the /ebay/boiler-save + /ebay/boiler-delete
# routes (Boiler Alarm Scanner, docs/boiler_alarm/), merge-safe.
# =============================================================================
# Builds the FULL Lambda package (lambda_function.py + every agent module the
# other routes import + vendored requests) exactly like deploy_big_blue_scanner.sh —
...
# are needed: GITHUB_TOKEN is already set on the live Lambda, and
# boiler-save/boiler-delete need nothing else.
#
...
# ("boiler-save"/"boiler-delete" are already in local.agent_routes in main.tf).
...
ZIP="$SCRIPT_DIR/.build/boiler_alarm_deploy.zip"
SMOKE_BODY="$(mktemp -t boiler_alarm_smoke.XXXXXX.json)"
...
echo ""
echo ">>> Smoke testing /ebay/boiler-save (dummy payload, expect success:false)..."
HTTP_STATUS=$(curl -s -o "$SMOKE_BODY" -w "%{http_code}" \
  -X POST "${API_BASE}/boiler-save" \
...
echo "  >>> register POST/OPTIONS /ebay/boiler-save + /ebay/boiler-delete in API"
