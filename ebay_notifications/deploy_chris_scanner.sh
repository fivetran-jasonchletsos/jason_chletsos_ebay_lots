#!/usr/bin/env bash
# =============================================================================
# deploy_chris_scanner.sh — ship the /ebay/chris-save + /ebay/chris-delete routes
# (Chris' Scanner, docs/chris_scanner/), merge-safe.
# =============================================================================
...
# deploy_natasha_pokedex.sh), and chris-save/chris-delete need nothing else.
#
...
# ("chris-save"/"chris-delete" are already in local.agent_routes in main.tf).
...
ZIP="$SCRIPT_DIR/.build/chris_scanner_deploy.zip"
SMOKE_BODY="$(mktemp -t chris_scanner_smoke.XXXXXX.json)"
...
echo ""
echo ">>> Smoke testing /ebay/chris-save (dummy payload, expect success:false)..."
HTTP_STATUS=$(curl -s -o "$SMOKE_BODY" -w "%{http_code}" \
  -X POST "${API_BASE}/chris-save" \
...
echo "  >>> register POST/OPTIONS /ebay/chris-save + /ebay/chris-delete in API"
