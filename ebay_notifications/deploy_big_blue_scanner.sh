#!/usr/bin/env bash
# =============================================================================
# deploy_big_blue_scanner.sh — ship the /ebay/rick-save + /ebay/rick-delete
# routes (Big Blue Scanner, docs/big_blue_scanner/), merge-safe.
# =============================================================================
# Builds the FULL Lambda package (lambda_function.py + every agent module the
# other routes import + vendored requests) exactly like deploy_chris_scanner.sh —
...
# rick-save/rick-delete need nothing else.
#
...
# ("rick-save"/"rick-delete" are already in local.agent_routes in main.tf).
...
# first deploy.
#
# Usage: ./deploy_big_blue_scanner.sh
...
ZIP="$SCRIPT_DIR/.build/big_blue_scanner_deploy.zip"
SMOKE_BODY="$(mktemp -t big_blue_scanner_smoke.XXXXXX.json)"
...
echo ""
echo ">>> Smoke testing /ebay/rick-save (dummy payload, expect success:false)..."
HTTP_STATUS=$(curl -s -o "$SMOKE_BODY" -w "%{http_code}" \
  -X POST "${API_BASE}/rick-save" \
...
echo "  >>> register POST/OPTIONS /ebay/rick-save + /ebay/rick-delete in API"
