"""_sig_roundpick_lots.py — 2025 Topps Signature Class "Round X Pick Y" (Purple/Green) lots.
Built from Scans 189-192.

  python3 _sig_roundpick_lots.py --pdf
  python3 _sig_roundpick_lots.py --apply [--only key1,key2]
"""
...
SET_LABEL = "2025 Topps Signature Class (Purple / Green)"
...
LOTS = [
    {"key": "rp_qb", "price": 17.99,
     "title": "2025 Topps Signature Class Round Pick QB Lot Rodgers Stroud Maye McCarthy Ward RC",
     "theme": "Quarterback Lot",
     "cards": [(192, 4, "Aaron Rodgers", "Steelers", False),
               (191, 6, "C.J. Stroud", "Texans", False),
               (191, 7, "Drake Maye", "Patriots", False),
               (189, 3, "J.J. McCarthy", "Vikings", False),
               (192, 2, "Cam Ward", "Titans", True)]},
    {"key": "rp_wr", "price": 18.99,
     "title": "2025 Topps Signature Class Round Pick WR Lot Chase Nacua Adams London Samuel Football",
     "theme": "WR Star Lot",
     "cards": [(190, 9, "Ja'Marr Chase", "Bengals", False),
               (192, 5, "Puka Nacua", "Rams", False),
               (189, 1, "Davante Adams", "Rams", False),
               (190, 6, "Drake London", "Falcons", False),
               (192, 1, "Deebo Samuel", "Commanders", False)]},
    {"key": "rp_rb", "price": 14.99,
     "title": "2025 Topps Signature Class Round Pick RB Lot Hampton Mixon James Mullings Pierce RC",
     "theme": "Running Back / Skill Lot",
     "cards": [(192, 8, "Omarion Hampton", "Chargers", True),
               (191, 8, "Joe Mixon", "Texans", False),
               (189, 2, "Jordan James", "49ers", True),
               (191, 1, "Kalel Mullings", "Titans", True),
               (190, 4, "Alec Pierce", "Colts", False)]},
    {"key": "rp_rookwr", "price": 12.99,
     "title": "2025 Topps Signature Class Round Pick Rookie WR Lot Odunze Pearsall Noel Horton Royals",
     "theme": "Rookie WR Lot",
     "cards": [(190, 5, "Rome Odunze", "Bears", False),
               (189, 5, "Ricky Pearsall", "49ers", False),
               (191, 4, "Jaylin Noel", "Texans", True),
               (192, 6, "Tory Horton", "Seahawks", True),
               (189, 6, "Jalen Royals", "Chiefs", True)]},
    {"key": "rp_te", "price": 14.99,
     "title": "2025 Topps Signature Class Round Pick TE Lot Bowers Njoku Ertz Helm Leonard RC Football",
     "theme": "Tight End + QB Lot",
     "cards": [(189, 9, "Brock Bowers", "Raiders", False),
               (190, 8, "David Njoku", "Browns", False),
               (189, 4, "Zach Ertz", "Commanders", False),
               (191, 9, "Gunnar Helm", "Titans", True),
               (192, 3, "Riley Leonard", "Colts", True)]},
    {"key": "rp_def", "price": 12.99,
     "title": "2025 Topps Signature Class Round Pick Defense Lot Garrett Graham Hassanein Green RC",
     "theme": "Defense / Trenches Lot",
     "cards": [(192, 7, "Myles Garrett", "Browns", False),
               (190, 1, "Mason Graham", "Browns", True),
               (190, 2, "Ahmed Hassanein", "Lions", True),
               (191, 5, "Mike Green", "Ravens", True),
               (191, 3, "Danny Stutsman", "Saints", True)]},
    {"key": "rp_rook", "price": 11.99,
     "title": "2025 Topps Signature Class Round Pick Rookie Lot McCarthy Gabriel Williams Moore RC",
     "theme": "Rookie Mix Lot",
     "cards": [(191, 2, "J.J. McCarthy", "Vikings", False),
               (189, 7, "Dillon Gabriel", "Browns", True),
               (189, 8, "Kyle Williams", "Patriots", True),
               (190, 3, "Malachi Moore", "Jets", True),
               (190, 7, "Jonah Savaiinaea", "Dolphins", True)]},
]
...
def do_apply(only=None):
    ...
    for lot in lots:
        ...
        desc = (
            ...
            f"<ul>{items}</ul>"
            "<ul>"
            "<li>2025 Topps Signature Class - Purple/Green torn design of each card.</li>"
        ...
        specifics = {"Sport": "Football", "Set": "2025 Topps Signature Class",
                     "Parallel/Variety": "Purple / Green", "Type": "Sports Trading Card Lot",
                     "Features": "Lot", "League": "National Football League (NFL)"}
        specifics_xml = "".join(
...
<AddItemRequest xmlns="{NS}">
  ...
  <Item>
    ...
    for theme, iid, price in results:
        print(f"  {theme:28s} ${price:6.2f}  {iid or 'FAILED'}")
...
def main():
    ...
    a = ap.parse_args()
    if a.pdf:
        out = Path("output/signature_class_roundpick_lots_pull_sheet.pdf")
        build_pdf(out)
        print(f"PDF: {out}")
    if a.apply:
