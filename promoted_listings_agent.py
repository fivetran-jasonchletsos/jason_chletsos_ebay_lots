...
def classify(listing: dict, *, age_days: int | None, watchers: int,
    ...
    # Price-band tiers are AUTHORITATIVE when configured (promo-burst mode):
    # the ad rate is driven by listing price/value, not age, so the visibility
    # push reaches every listing. The no_ad guards above still exempt items
    # that are already selling. Remove price_band_tiers from config to revert
    # to the age-based ladder (steady-state 8% cap).
        ...
        try:
            ...
            for band in sorted(price_bands, key=lambda b: b["max_price"]):
                if price <= band["max_price"]:
                    reasons.append(f"price ${price:.2f} → price-band tier {band['tier']}")
