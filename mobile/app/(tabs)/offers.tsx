 ...
 * Each action confirms before hitting the LIVE eBay Trading API.
 *
...
export default function OffersScreen() {
  ...
  function confirmAction(offer: BestOffer, action: OfferAction) {
    const summary = action === 'Accept'
      ? `Accept ${offer.buyer_user_id}'s offer of $${offer.offer_price.toFixed(2)} on "${offer.item_title.slice(0, 40)}…"? This will SELL the item at that price.`
  ...
  function openCounter(offer: BestOffer) {
    ...
    // Default counter midway between offer and list price, rounded to $0.99.
    const list = offer.item_price ?? offer.offer_price;
    const mid = (offer.offer_price + list) / 2;
    const suggested = Math.max(0.5, Math.round(mid) - 0.01);
      ...
      <FlashList
        ...
        ListEmptyComponent={() => (
          <View style={[styles.center, { paddingTop: 80 }]}>
            ...
            <Text style={[styles.muted, { fontSize: 11, marginTop: 6 }]}>Pull to refresh.</Text>
          </View>
        )}
      />
        ...
        <Pressable style={styles.modalBackdrop} onPress={() => setCounterTarget(null)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <View style={styles.modalCompareRow}>
              <Compare label="List" value={counterTarget?.item_price != null ? `$${counterTarget.item_price.toFixed(2)}` : '—'} />
              <Compare label="Offer" value={counterTarget != null ? `$${counterTarget.offer_price.toFixed(2)}` : '—'} highlight />
            </View>
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 14 }}>
              <TouchableOpacity style={[styles.btnGold, { flex: 1 }]} onPress={submitCounter} disabled={submitting}>
                <Text style={styles.btnGoldText}>Send counter</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btnGhost, { flex: 1 }]} onPress={() => setCounterTarget(null)} disabled={submitting}>
  ...
  return (
    <View style={styles.row}>
      ...
      <View style={styles.compareRow}>
        <Compare label="List" value={offer.item_price != null ? `$${offer.item_price.toFixed(2)}` : '—'} />
        <Compare label="Offer" value={`$${offer.offer_price.toFixed(2)}`} highlight />
        <Compare label="Discount" value={discount != null ? `${discount}%` : '—'} />
      </View>

      <View style={styles.actions}>
        <TouchableOpacity style={[styles.btnAccept, disabled && { opacity: 0.5 }]} onPress={onAccept} disabled={disabled}>
...
function Compare({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <View style={styles.compareCell}>
      <Text style={styles.compareLabel}>{label}</Text>
      <Text style={[styles.compareValue, highlight && styles.compareValueHi]}>{value}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  compareCell: { flex: 1 },
  compareLabel: { color: theme.textDim, fontSize: 9, fontWeight: '800', letterSpacing: 1.5, textTransform: 'uppercase' },
  compareValue: { color: theme.text, fontSize: 16, fontWeight: '800', marginTop: 2 },
  compareValueHi: { color: theme.goldBright },

  actions: { flexDirection: 'row', gap: 8, marginTop: 12 },
  btnAccept: { flex: 1, backgroundColor: theme.gold, paddingVertical: 11, borderRadius: radii.sm, alignItems: 'center' },
